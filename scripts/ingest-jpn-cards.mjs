...
const EXPANSIONS_ENDPOINT_PREFIX = "/pokemon/v1/ja/expansions";
const EXPANSIONS_TABLE = "jpn_expansions";
const CARDS_TABLE = "jpn_cards";
...
async function fetchOfflineExpansions(supabaseRead, startSet, maxSets) {
  ...
  if (error) {
    throw new Error(`Failed to load Japanese expansions from Supabase: ${error.message}`);
  }
...
async function run() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/ingest-jpn-cards.mjs [--dry-run] [--page-size=100] [--start-set=1] [--max-sets=20] [--max-pages=50]",
    );
    process.exit(0);
  }
  ...
  if (selected.length === 0) {
    console.log("No eligible Japanese expansions found for ingest.");
    return;
  }

  console.log(
    `Using ${selected.length} Japanese expansion(s) from Supabase (offline only). Total available offline expansions: ${totalAvailable}`,
  );
  ...
  console.log(
    args.dryRun ? "Japanese cards dry run complete." : "Japanese cards ingest complete.",
  );
