...
const EXPANSIONS_TABLE = "expansions";
...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    if (arg === "--help" || arg === "-h") out.help = true;
      ...
      out.expansionId = arg.split("=")[1]?.trim() ?? "";
...
async function fetchSetCardsPage({ apiKey, teamId, expansionId, page, pageSize }) {
  const url = new URL(
    `${SCRYDEX_BASE_URL}${EXPANSIONS_ENDPOINT_PREFIX}/${expansionId}/cards`,
  );
  ...
  if (!res.ok) {
    throw new Error(
      `Scrydex ${res.status} on expansion ${expansionId}, page ${page}: ${text}`,
    );
  }
...
async function upsertCardsBatch(supabase, rows, expansionId, page) {
  ...
  if (error) {
    throw new Error(
      `Supabase upsert failed for expansion ${expansionId}, page ${page}: ${error.message}`,
    );
  }
}

async function fetchExpansionMeta(supabaseRead, expansionId) {
  const { data, error } = await supabaseRead
    .from(EXPANSIONS_TABLE)
    .select("id,name,is_online_only")
    .eq("id", expansionId)
    .maybeSingle();

  if (error) {
    throw new Error(`Failed to load expansion ${expansionId} from Supabase: ${error.message}`);
  }

  if (!data) {
    throw new Error(
      `Expansion ${expansionId} was not found in Supabase. Run npm run ingest:expansions first.`,
    );
  }

  if (data.is_online_only === true) {
    throw new Error(`Expansion ${expansionId} is marked online-only and will not be ingested.`);
  }
...
async function run() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/ingest-cards-by-expansion.mjs --expansion-id=sv10 [--dry-run] [--page-size=100] [--max-pages=50]",
    );
    process.exit(0);
  }
  ...
  const expansion = await fetchExpansionMeta(supabaseRead, args.expansionId);
  ...
  while (true) {
    ...
    for (const card of data) {
      const row = toCardRow(card, expansion.id, expansion.name);
      if (!row) {
        if (
          card &&
          typeof card === "object" &&
    ...
    console.log(
      `${expansion.id} page ${page}: raw=${data.length}, kept=${transformed.length}, running kept=${keptCardCount}`,
    );

    if (!args.dryRun && transformed.length > 0) {
      await upsertCardsBatch(supabaseWrite, transformed, expansion.id, page);
  ...
  console.log(
    args.dryRun
      ? `Expansion ${expansion.id} dry run complete.`
      : `Expansion ${expansion.id} ingest complete.`,
  );
