...
const EXPANSIONS_ENDPOINT = "/pokemon/v1/ja/expansions";
const SUPABASE_TABLE = "jpn_expansions";
...
async function run() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/ingest-jpn-expansions.mjs [--dry-run] [--page-size=100] [--max-pages=2]",
    );
    process.exit(0);
  }
  ...
  while (true) {
    ...
    console.log(
      `Fetched page ${page}: ${validRows.length} Japanese expansions (running total: ${collected.length})`,
    );
  ...
  if (collected.length === 0) {
    console.log("No Japanese expansions returned. Nothing to upsert.");
    return;
  }

  if (args.dryRun) {
    console.log(`Dry run complete. Would upsert ${collected.length} Japanese expansions.`);
  ...
  console.log("Japanese expansions ingest complete.");
  console.log(`Japanese expansions upserted: ${upserted}`);
