...
export default function QuickJumpSets({ sets, onJump }: QuickJumpSetsProps) {
  return (
    <div className="flex w-full flex-col gap-2">
      <div className="text-xs font-nunito font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
        Quick Jump Sets
      </div>
        ...
        <div className="flex flex-col items-start gap-2.5">
          {sets.map((set) => (
            <button
              ...
              className="text-xs text-left font-nunito font-medium text-zinc-700 dark:text-slate-100">
              {set.name}
            </button>
          ))}
        </div>
